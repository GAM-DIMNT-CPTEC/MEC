...
for (thres in thresholds) {
  ...
  EVAL.CAT.PER[.(reg, mask, CFG$dt, CFG$hh, thres), c("HITS","FAL_AL","MISS","COR_NEG") := as.list(EV_CAT)]
  if (is.na(EVAL.CAT.PER[.(reg, mask, CFG$dt, CFG$hh, thres), HITS]) ||
        is.infinite(EVAL.CAT.PER[.(reg, mask, CFG$dt, CFG$hh, thres), HITS])) {
      cat("\n* ERR EvalCatPer.R:  metric=", st_name, "  OBS=", mean(vOBS), "  FCT=", mean(vFCT))
  }
