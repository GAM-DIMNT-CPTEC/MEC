...
# ******* ABORTED - CANT ESTABLISH CI DOING SO *******
# Substituting categorical metrics to be only the contingency table values
# So, metrics should be calculated on-the-fly,
...
if (CFG.SET$eval.cat) {
  ...
  if (!file.exists(fname.eval.cat)) {
    # Só a região maior (a primeira) é sem máscara, caso seja para avaliar sem máscara
    if ("ALL" %in% CFG.SET$masks) {
      comm = paste0("EVAL.CAT <- data.table(expand.grid(REG=CFG$area1.names[1], MASK='ALL', DT=CFG$dates, HH=CFG$hh, THRES=thresholds, ", paste0(c(METRICS$CAT, paste0(c("LCI95","UCI95"), "_", rep(METRICS$CAT, each=2))), "=NA_real_", collapse=", "), "))")
      eval(parse(text=comm))
    } else {
      EVAL.CAT = data.table()
    }
    # Acrescenta todas as regiões somente com a máscara CONT
    comm = paste0("EVAL.CAT <- rbind.data.frame(EVAL.CAT, data.table(expand.grid(REG=c(CFG$area1.names, REGIONS$NAME), MASK='CONT', DT=CFG$dates, HH=CFG$hh, THRES=thresholds, ", paste0(c(METRICS$CAT, paste0(c("LCI95","UCI95"), "_", rep(METRICS$CAT, each=2))), "=NA_real_", collapse=", "), ")))")
    eval(parse(text=comm))
