...
for (thres in thresholds) {
  ...
  EVAL.CAT[.(reg, mask, CFG$dt, CFG$hh, thres), c("HITS","FAL_AL","MISS","COR_NEG") := as.list(EV_CAT)]
  if (is.na(EVAL.CAT[.(reg, mask, CFG$dt, CFG$hh, thres), HITS]) ||
        is.infinite(EVAL.CAT[.(reg, mask, CFG$dt, CFG$hh, thres), HITS])) {
