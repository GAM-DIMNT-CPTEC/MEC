...
if (CFG.SET$eval.cat) {
  ...
  for (ltime.num in 1:ltime2) {
    ...
    if (length(files)>0) {
      ...
      #               paste0(METRICS$CT, "=NA_integer_", collapse=", "), ")")
      comm = paste0("CAT.UPDATE <- data.table(EV_SET=character(num.lin), MOD=character(num.lin), LTIME=ltime.num, REG=character(num.lin), MASK=character(num.lin), DT=integer(num.lin), HH=CFG$hh, THRES=numeric(num.lin), ", paste0(METRICS$CT, "=NA_integer_", collapse=", "), ")")
      ...
      CAT.UPDATE <- CAT.UPDATE[!(is.na(HITS) | is.na(FAL_AL) | is.na(MISS) | is.na(COR_NEG)) & DT <= MEC_last_dt]
